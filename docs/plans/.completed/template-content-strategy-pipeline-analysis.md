...
1. **W1 step 4 is already handled; W1 step 5 is broken.**
   ...
   - **Step 5 (build hierarchy):** the `-TriggerProjectSetup $True` path (default)
     ...
     incompatible with `gap-miner-v2-quebec50`'s new structure. The user passes
...
3. **NEW bug: AGENTS.md semantic rewrite silently fails.** The script targets
   ...
   replacement did land (`nam20485/gap-miner-v2-quebec50` → the clone name), but the
...
- **W1.5 (BROKEN — needs replacement):** `-TriggerProjectSetup $True` currently
    ...
    template and is incompatible with `gap-miner-v2-quebec50`'s new structure (the user
...
**In this template repo (`nam20485/gap-miner-v2-quebec50`):**
...
./scripts/create-repo-from-slug.ps1 `
  -Slug "gap-miner-v2" -TemplateRepoName "gap-miner-v2-quebec50" `
...
| Step | Description |
| --- | --- |
| Create repo | `gh repo create --template nam20485/gap-miner-v2-quebec50` |
...
- **W1 step 5 (build hierarchy) trigger is broken.** `-TriggerProjectSetup $True`
  ...
  incompatible with `gap-miner-v2-quebec50`'s new structure. The user already bypasses
...
- **AGENTS.md rewrite bug (NEW, 2026-07-19).** The script targets
  ...
  (`nam20485/gap-miner-v2-quebec50` → the clone name), but the role label was not
